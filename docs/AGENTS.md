...
- Use explicit `@importFrom` statements with package version
  requirements
...
- Required tags: `@title`, `@name`, `@description`, `@export`, `@param`,
  `@returns`
- Include `@examples` with `\dontrun{}` wrapper for examples that
...
- Use [`stop()`](https://rdrr.io/r/base/stop.html) for unrecoverable
...
- Use [`log2()`](https://rdrr.io/r/base/Log.html) and `2 ^` for
  cent/ratio conversions
...
- Prefix helper functions with `.` (e.g., `.ratio2cents`,
  `.cents2ratio`)
- Keep helper functions in the same file as main functions
- Document helpers with
  [`utils::globalVariables()`](https://rdrr.io/r/utils/globalVariables.html)
  if they’re not exported

## Testing

### Test Structure

- This package uses manual testing via examples rather than automated
  testthat
...
- Split code by functionality: `cps_chord_table.R`, `keyboard_map.R`,
  etc.
...
- Order functions logically: data creation -\> transformation -\>
  analysis
...
This document provides comprehensive guidelines for developing and
maintaining the eikosany R package. Follow these conventions to ensure
