...
split_tetrad <- function(tetrad) {
  as.numeric(unlist(strsplit(tetrad, ":")))
}

tetrad2note_numbers <- function(tetrad, harmonics, scale) {
  notes <- split_tetrad(tetrad)
...
tetrad2keys <- function(tetrad, harmonics, scale) {
  note_numbers <- split_tetrad(tetrad2note_numbers(
    tetrad, harmonics, scale
  ))
...
sub_tetrad2keys <- function(tetrad, scale) {
  note_numbers <- split_tetrad(sub_tetrad2note_numbers(
    tetrad, scale
  ))
...
eikosany <- eikosany[, .(
  cps_label, ratio = octave_reduce(as.bigq(product) / big15)
  )]
eikosany <- eikosany[, .(
  cps_label,
...
harmonic_tetrads <- tetrads[, .(
  tetrad_label,
  note_numbers = ":",
  keys = ":"
  )]
for (it in 1:nrow(harmonic_tetrads)) {
  harmonic_tetrads$keys[it] <-
    tetrad2keys(
...
sub_harmonic_tetrads <- tetrads[, .(
  tetrad_label,
  sub_note_numbers = ":",
  sub_keys = ":")]
for (it in 1:nrow(sub_harmonic_tetrads)) {
  sub_harmonic_tetrads$sub_keys[it] <-
    sub_tetrad2keys(
