...
note_numbers <- 60:(60 + length(key_names) - 1)
CHOOSE <- 3
...
eikosany <- eikosany[, .(
  cps_label = paste(V1, V2, V3, sep = "*"),
  ratio = octave_reduce(as.bigq(V1 * V2 * V3) / NORMALIZER)
  )]
eikosany <- eikosany[, .(
  cps_label,
...
eikosany <- eikosany[, .(
  ...
  midi_note_number = note_numbers[1:scale_steps],
  cps_label
  )]
...
```{r}
chords <- as.data.table(t(combn(harmonics, 4)))[, .(
